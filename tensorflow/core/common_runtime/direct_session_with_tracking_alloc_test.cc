  ...
  for (auto& it : cost_models) {
    ...
    for (Node* node : g->nodes()) {
      if (node->name() == y->name() || node->name() == y_neg->name()) {
...
#else
        // if MKL (MKL_ML/MKL_DNN) is used, it goes through various additional 
        ...
        // MKL is used, they can be 9, 10 and 11 or 
        // other numbers. Thus if MKL is used
        // following check will not hold. 
        // Thus skipping the check if MKL is used.
        if (node->name() == y->name()) {
          EXPECT_LE(3, cm->AllocationId(node, 0));
        } else {
          EXPECT_LE(4, cm->AllocationId(node, 0));
        }
#endif
      }
